...
- `app-v2.0.3.apk`: current 2.0.3 package. It repairs `adb reverse` after a real phone is unplugged and reconnected, clears stale connection badges, retries unavailable routes within 10 seconds, hides processing status from the learning screen, defaults sentence playback to single, preserves final-word audio without bleeding into the next sentence, and fixes `calling -> call` lookup. Pair it with `tools/start_video_english_service_v2.0.2.ps1` and `tools/local_whisper_service_v2.0.2.py`.
...
```powershell
Copy-Item app/build/outputs/apk/debug/app-debug.apk release/app-v2.0.3.apk -Force
$hash = Get-FileHash -Algorithm SHA256 release/app-v2.0.3.apk
"$($hash.Hash.ToLower())  app-v2.0.3.apk" | Add-Content -Encoding UTF8 release/SHA256SUMS.txt
```

Install with adb:

```powershell
adb install -r release/app-v2.0.3.apk
```
