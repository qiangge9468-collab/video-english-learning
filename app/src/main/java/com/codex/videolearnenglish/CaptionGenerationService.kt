...
class CaptionGenerationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        Thread {
            ...
            try {
                ...
                active = false
                currentTaskId = null
                val continuingQueue = startNextQueuedTask()
                if (!continuingQueue) {
                    releaseWakeLock()
                    releaseWifiLock()
    ...
    private fun pollTranscriptJob(
        ...
        while (true) {
            ensureTaskActive()
            if (CaptionTaskStore.hasPendingUpload(this, taskId)) {
                CaptionTaskStore.markSubmitted(this, taskId, jobId, serviceLabel(serviceUrl))
                updateNotification("发现新视频，优先提取并上传音频")
                sendProgress("电脑任务已保存并会独立继续；现在优先上传新加入的视频。")
                return null
            }
            val now = System.currentTimeMillis()
