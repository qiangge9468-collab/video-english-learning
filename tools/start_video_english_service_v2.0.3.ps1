...
$runtimeConfig = Join-Path $projectRoot "tools\runtime_service_config_v2.0.3.json"
$runtimeStatus = Join-Path $projectRoot "tools\runtime_status_v2.0.3.json"
$latestUrlsFile = Join-Path $projectRoot "tools\latest_service_urls_v2.0.3.txt"
...
function Write-ServiceDashboard {
    ...
    if (-not $script:dashboardHeaderPrinted) {
        Write-Host ""
        Write-Host "看视频学英语 v2.0.3 - 电脑端服务已启动" -ForegroundColor Cyan
        Write-Host "============================================================"
        Write-Host "连接地址"
...
if (Test-PortBusy $Port) {
    ...
    Write-Host "  powershell -ExecutionPolicy Bypass -File tools/start_video_english_service_v2.0.3.ps1 -Port 8767"
    exit 1
}
...
$serviceJob = Start-Job -ArgumentList $projectRoot, $Port, $token, $runtimeConfig, $runtimeStatus, $env:PATH, $env:TRANSLATION_PROVIDER, $env:TRANSLATION_MODEL, $env:TRANSLATION_DEVICE, $env:TRANSLATION_SOURCE_LANGUAGE, $env:TRANSLATION_TARGET_LANGUAGE, $env:TRANSLATION_STYLE, $env:WHISPER_DEVICE, $env:WHISPER_COMPUTE_TYPE, $env:WHISPER_ENGLISH_MODEL, $env:WHISPER_HOTWORDS, $env:WHISPER_INITIAL_PROMPT -ScriptBlock {
    ...
    $env:VIDEO_ENGLISH_DATA_DIR = Join-Path $Root "service_data_v2.0.3"
    ...
    & $servicePythonExe tools\local_whisper_service_v2.0.3.py
}
