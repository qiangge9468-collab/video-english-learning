...
try:
    from durable_job_store_v2_0_3 import DurableJobStore
    from semantic_caption_segmenter import SegmenterConfig, segment_words as semantic_segment_words
except ModuleNotFoundError:
    from tools.durable_job_store_v2_0_3 import DurableJobStore
    from tools.semantic_caption_segmenter import SegmenterConfig, segment_words as semantic_segment_words
...
SERVICE_DATA_DIR = os.environ.get(
    "VIDEO_ENGLISH_DATA_DIR",
    os.path.join(PROJECT_ROOT, "service_data_v2.0.3"),
)
...
def default_runtime_status():
    return {
        ...
        "port": os.environ.get("WHISPER_PORT", "8766"),
...
def service_config():
    current_port = int(os.environ.get("WHISPER_PORT", "8766"))
    ...
    config = {
        ...
        "segmenter": {
            ...
            "enabled": semantic_segmenter_enabled(),
            "sat_model": semantic_sat_model_path(),
...
def semantic_segmenter_enabled():
    return env_enabled("SUBTITLE_SEMANTIC_ENABLED", True)


def semantic_sat_model_path():
    return os.environ.get(
        "SAT_MODEL_DIR",
        os.path.join(MODELS_DIR, "sat-12l-sm"),
    )


def semantic_tokenizer_path():
    return os.environ.get(
        "SAT_TOKENIZER_DIR",
        os.path.join(MODELS_DIR, "xlm-roberta-base"),
    )
...
def semantic_segmenter_config():
    return SegmenterConfig(
        ...
        max_seconds=MAX_SUBTITLE_SECONDS,
        preferred_max_words=PREFERRED_MAX_SUBTITLE_WORDS,
        max_words=MAX_SUBTITLE_WORDS,
        preferred_max_chars=PREFERRED_MAX_SUBTITLE_CHARS,
        max_chars=MAX_SUBTITLE_CHARS,
        soft_silence_seconds=SOFT_SILENCE_SECONDS,
        silence_break_seconds=SILENCE_BREAK_SECONDS,
        absolute_silence_seconds=ABSOLUTE_SILENCE_SECONDS,
        orphan_probability_threshold=float(
            os.environ.get("SUBTITLE_ORPHAN_PROBABILITY", "0.58")
        ),
    )


def semantic_python_path():
    return os.environ.get(
        "SEMANTIC_PYTHON",
        r"D:\anaconda\envs\subtitle\python.exe",
    )


def run_semantic_worker(raw_words):
    python_path = semantic_python_path()
    worker_path = os.path.join(PROJECT_ROOT, "tools", "semantic_segmenter_worker_v2_0_3.py")
    if not os.path.isfile(python_path):
        raise FileNotFoundError(f"Semantic Python is missing: {python_path}")
    if not os.path.isfile(worker_path):
        raise FileNotFoundError(f"Semantic worker is missing: {worker_path}")

    os.makedirs(SERVICE_DATA_DIR, exist_ok=True)
    input_fd, input_path = tempfile.mkstemp(
        prefix="semantic_input_", suffix=".json", dir=SERVICE_DATA_DIR
    )
    output_fd, output_path = tempfile.mkstemp(
        prefix="semantic_output_", suffix=".json", dir=SERVICE_DATA_DIR
    )
    os.close(output_fd)
    try:
        with os.fdopen(input_fd, "w", encoding="utf-8") as handle:
            json.dump(
                {
                    "words": raw_words,
                    "config": semantic_segmenter_config().__dict__,
                },
                handle,
                ensure_ascii=False,
            )
        ...
        command = [
            ...
            output_path,
            "--sat-model",
            semantic_sat_model_path(),
            "--tokenizer",
            semantic_tokenizer_path(),
            "--spacy-model",
            os.environ.get("SPACY_MODEL", "en_core_web_trf"),
        ]
        timeout = int(os.environ.get("SEMANTIC_WORKER_TIMEOUT_SECONDS", "900"))
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError(f"semantic worker exited {completed.returncode}: {detail[-2000:]}")
        with open(output_path, "r", encoding="utf-8-sig") as handle:
            payload = json.load(handle)
        segments = payload.get("segments")
        debug = payload.get("debug")
        if not isinstance(segments, list) or not isinstance(debug, dict):
            raise RuntimeError("semantic worker returned an invalid payload")
...
def save_artifact(audio_hash, name, value):
    if audio_hash:
...
def transcribe(
            ...
            raw_segments.append(
                {
                    ...
                    "word_start": segment_word_start,
                    "word_end": len(raw_words),
                    "translation": "",
                }
            )
    ...
    save_artifact(artifact_audio_hash, "english.legacy.json", legacy)
    ...
    if raw_words and semantic_segmenter_enabled():
        try:
            report(progress, "postprocessing", 89, "Loading semantic English segmenter")
            report(progress, "postprocessing", 90, "Optimizing semantic subtitle boundaries")
            semantic, debug = run_semantic_worker(raw_words)
            semantic = finalize_english_segments(semantic)
            result = semantic
            debug["used_fallback"] = False
            debug["service_version"] = SERVICE_VERSION
            save_artifact(artifact_audio_hash, "english.semantic-preview.json", semantic)
        except Exception as exc:
            debug["error"] = f"{type(exc).__name__}: {exc}"
            print(f"Semantic segmenter failed; using legacy captions: {exc}", flush=True)
            if not env_enabled("SUBTITLE_SEMANTIC_FALLBACK", True):
                raise
    save_artifact(artifact_audio_hash, "segmentation_debug.json", debug)
...
def main():
    ...
    port = int(os.environ.get("WHISPER_PORT", "8766"))
    configured_english_model = os.environ.get("WHISPER_ENGLISH_MODEL", "large-v3")
