...
“生成字幕中”页面支持一次选择多个视频排队处理。App 会按顺序提取音频并分块上传，然后保存电脑端任务 ID、阶段进度和最后同步时间。电脑开始处理后不再依赖手机保持长连接；手机恢复网络时会自动继续查询，任务完成后再下载双语结果。
...
v2.0.2 是当前推荐版本，重点解决长视频上传中断、手机短暂离线导致任务失败，以及重复生成浪费上传时间的问题。
...
- 任务卡显示上传进度、电脑端阶段进度、任务 ID 和最后同步时间。
...
4. 电脑创建持久化任务并独立运行 Whisper 识别和中文翻译。
5. 手机定期查询任务阶段、已处理时长和总进度，断线时自动退避重连。
6. 电脑保存英文与双语结果；手机校验每条英文都有中文翻译后，再原子替换本地旧字幕。

音频完整上传并取得电脑任务 ID 后，手机不需要保持持续连接。电脑会继续处理；手机再次联网或重新打开 App 后，会使用原任务 ID 继续显示进度和取回结果。
...
这个页面专门负责后台生成字幕和翻译。你可以一次添加多个视频，它们会排队处理。每个任务会显示视频首帧、文件名、总进度、上传字节进度、电脑端阶段、电脑任务 ID、最后同步时间、连接方式和操作按钮。
...
手机侧任务在通知栏以前台服务持续运行，可以正常锁屏或切换到其他应用。上传完成后，电脑任务与手机连接解耦；暂时断网只会让手机显示“等待连接电脑”，不会终止电脑识别或翻译。恢复连接后自动继续查询。部分国产系统仍建议允许本 App 后台活动并关闭省电限制。
...
如果你只是安装使用，推荐下载 `release/app-v2.0.2.apk`。如果你想自己改代码，可以 clone 项目后用 Android Studio 打开。
